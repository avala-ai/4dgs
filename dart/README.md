...
(spec section 5.15) and the object layer (sections 5.15.6-5.15.7) — so every decode row in the
[feature matrix](../website/docs/reference/index.md) is filled in from a suite that runs. It
declines the invalid corpus's refusal expectations, which is what the remaining `No` cells in that
table record.
