...
ordinary `gaussian-birth` chunks may; the promise is coverage, not tiling. The promise also imposes
one conditional physical-order rule so that coverage remains streamable: considering only level-0
Chunk records in increasing file-offset order, their `t0` values MUST be non-decreasing.
Higher-level chunks and non-Chunk records may still be interleaved freely.

A front-to-back reader validates the promise with constant state. Before any comparison, it MUST
require finite Header `duration_sec` and finite `t0` and `t1` on every level-0 Chunk, refusing
`lod-level0-coverage-non-finite` and naming the Header field or chunk offset otherwise. It starts
`level0CoveredUntil = 0`; for each finite level-0 Chunk it first refuses a `t0` lower than the
...
algorithm to index entries ordered by `chunk_offset`.

Independent of the completeness promise, every indexed file with a valid Header `lod_levels` gate
MUST store `t0` and `t1` in each Chunk Index entry bit-for-bit identically to the fields in the
Chunk or Delta Chunk it points at. An indexed decode verifies that equality whenever it fetches a
selected record; the exhaustive validator verifies every entry. A mismatch is
`level-index-interval-mismatch` and names the entry, record offset and both values.
...
ordering rule is conditional on the Header completeness promise; ordinary `gaussian-birth` files and
level 0 without that promise retain the base format's order independence. Endpoint integrity is
instead conditional only on the Header LOD gate, because every declared-LOD seek uses the indexed
interval predicate whether or not level 0 claims complete coverage.
...
Before sorting or comparing any run, a reader MUST require finite `t0` and `t1` on every
`keyframe-delta` state chunk at every level of a declared-LOD file. If
`lod_level0_complete = "true"`, Header `duration_sec` MUST also be finite before it is compared with
the run endpoints. This applies whether the state chunk is in level 0 or a sparse enhancement level
and whether the completeness key is true, false or absent. A violation is
`lod-level-interval-non-finite` and names the Header field or the level, record offset and endpoint.
In an indexed file, the same check applies to every Chunk Index entry while the small index is
...
duplicated facts (spec §5.8). Under the same valid gate, the index intervals are also duplicated
seek facts whether `lod_level0_complete` is `"true"`, `"false"` or absent: §3.5 requires each
entry's `t0` and `t1` to match its pointed Chunk or Delta Chunk. Without the valid gate, the current
schema contains no index `level`: a conforming producer MUST NOT emit that field, and a reader MUST
NOT interpret any trailing bytes as it. A malformed `lod_levels` value is refused as a malformed
gate (§9), not used to guess the field's presence. A streamed file with no Chunk Index still filters
on each Chunk or Delta Chunk's existing `level` as records arrive.
...
- **Header `lod_level0_complete`** — `"true"` when level 0 alone covers the timeline, so a reader
  ...
  cover `[0, duration_sec)` without a gap, MAY contain overlaps, and MUST appear with non-decreasing
  `t0` in the level-0 Chunk subsequence so a pipe reader can validate that union in constant memory
  (§3.5); Header `duration_sec` and every level-0 interval endpoint MUST be finite. Under
  `keyframe-delta`, level 0 MUST be one non-overlapping run from `0` through `duration_sec`. An
  absent Header key or Header value `"false"` means level 0 is not guaranteed to be standalone. When
  present in the Header, the value grammar is exactly the lowercase ASCII string `"true"` or
  `"false"`, with no whitespace; every other value, including `"TRUE"` and `"1"`, MUST be refused as
...
| a declared-LOD index interval differs from its pointed Chunk / Delta Chunk's               | indexed and streamed seeks would describe different physical sequences    |
...
| a promised `gaussian-birth` coverage operand is NaN or infinite                            | comparisons and `max` disagree across language runtimes                   |
| promised `gaussian-birth` level-0 Chunks descend in `t0` physical order                    | a pipe reader cannot validate arbitrary interval order with bounded state |
...
The `gaussian-birth` finiteness refusal is checked before ordering or coverage arithmetic; the order
refusal is checked before the coverage marker is advanced for a chunk. Both apply only when Header
`lod_level0_complete = "true"` and do not add restrictions to another file. With finite,
non-decreasing `t0`, §3.5's single `level0CoveredUntil` marker proves gap-free coverage without
retaining intervals, so both a pipe reader and an indexed reader can enforce the same promise with
bounded state.

The `keyframe-delta` finiteness refusal is broader because every declared-LOD state interval enters
the per-level sort and run classification, including sparse higher levels and files without a
...
| `LodLevel0CompleteBirth`   | `gaussian-birth`; ordered overlapping level-0 intervals whose union covers the clip  | constant-state coverage proof; overlap remains valid (§3.5, §4.4)                        |
...
| `LodMetadataOnlyLevels`    | Metadata `lod_levels = "2"`; Header gate and index `level` append absent             | Metadata cannot activate filtering or make the index append physically present           |
| `LodMetadataOnlyComplete`  | Metadata `lod_level0_complete = "TRUE"`; Header completeness key absent              | non-canonical Metadata value is legal, inert and cannot affect level-0 completeness      |
...
`LodMetadataOnlyLevels` carries informational chunk levels 0 and 1 but no Header gate and no LOD
index append. It MUST expose no canonical `lod` summary, perform no level-filtered read, and decode
the full scene exactly like `LodNoDeclare`; attempting to parse an appended index `level` fails the
expected request/record trace. `LodMetadataOnlyComplete` MUST produce the same completeness state as
an otherwise identical file with no completeness key: no Header promise is visible, no value grammar
is applied to the Metadata entry, and no level-0 coverage guarantee is inferred.
...
streamed validation. `LodKfDeltaNonFiniteInterval` generates quiet NaN, positive infinity and
negative infinity in each of one state chunk's `t0` and `t1`, first at level 0 with the completeness
key absent and then in a sparse higher level while level 0 is valid and complete. It also generates
all three non-finite Header `duration_sec` values with completeness true. Every form MUST produce
...
form changes A's indexed interval from `[0, 1)` to `[0, 0.5)` while a second level-0 Chunk B still
covers `[0, 1)`. Exhaustive validation MUST refuse the A mismatch. A point seek at `t = 0.75` is
deliberately not a refusal expectation: A deselects itself, and the request trace MUST show that the
indexed decoder does not fetch it to perform a whole-file audit. This fixture narrows path
equivalence to conforming files rather than letting conformance smuggle a payload scan into the
...
- Level seek and gated index field — filter by `level ≤ N` and decode the append only when Header
  `lod_levels` declares it; every declared-LOD index interval matches its pointed record, with full
  validation checking every entry and a point seek checking selected records only; a Metadata-only
  key leaves both semantics and index layout unchanged (§4.1, §4.2)
...
- Level-0 completeness — canonical Header promise grammar and model-specific coverage;
  ...
  and gap-free union with constant state while ignoring interleaved higher levels; canonical
  `"false"` leaves incomplete level 0 valid and Metadata-only keys are inert
...
- Keyframe-delta full-file identity — every id has one immutable level, active unions have unique
  ...
  proves validation continues after the configured in-memory capacity is exceeded, and reuse is
  exercised through both delta births and later keyframe state
...
| 5   | Must every level cover the whole timeline          | No; only Header `lod_level0_complete` promises level 0: finite coverage operands, non-decreasing `t0` and gap-free interval union for `gaussian-birth`, one full non-overlapping run for `keyframe-delta`                 | §3.5, §4.4, §9      |
