...
quality review — need and that the rest of the format does not express. Three of the family's
opcodes are defined here; the rest stay reserved (§5.15.5).
...
| `0x23`–`0x2F` | —            | §5.15.5 | reserved    |

The assignment runs in dependency order, lowest first: a sensor's extrinsic and a rig's pose are
poses **in** a frame, and `0x20` is the record that names that frame. Nothing requires a reader to
see them in that order — records are skipped and dispatched by opcode, not by position — but a
writer that emits them in ascending opcode order produces a file whose front matter reads in the
order a human would explain it.

These records are **provisional** in the sense of §4.4: they are not frozen, and their fields may
change before version 1 is declared stable. They may only be extended by appending, like every
other record.
...
timing master — so a reader must know the answer *before* it can interpret a time, and the Header is
...
u8   geodetic_present  -- 0 or 1
     -- the following four fields are present only when geodetic_present is 1
f64  latitude_deg      -- WGS-84 geodetic latitude of the scene origin, degrees
f64  longitude_deg     -- WGS-84 longitude of the scene origin, degrees
f64  altitude_m        -- height of the scene origin above the WGS-84 ellipsoid, metres
...
**When `length_unit` and `metres_per_unit` disagree, `metres_per_unit` wins.** A writer MUST make
them agree — a file that declares `metre` and `0.01` is malformed and a validator SHOULD say so —
but a reader needs a rule for the file it has rather than the file it wanted, and the number is what
a consumer computes with while the enum is what it prints.

`geodetic_present` is `0` in a scene with no georeference, and the four anchor fields are then
**absent from the record, not zeroed**. A reader that has read `geodetic_present` knows exactly how
many bytes follow, so appended fields from a future revision sit at a position the reader can still
compute; this is the one place in the format where a field's presence depends on an earlier one, and
it is bounded to those four fields.

The anchor places the **scene origin** — the point `(0, 0, 0)` in file coordinates — on the WGS-84
ellipsoid, and `heading_deg` states where the frame's `forward_axis` points. Together with
`metres_per_unit` that is enough to put the scene on a map; it is deliberately not a full geodetic
transform, and a producer needing one should carry it in a Metadata record or an Attachment rather
than expect this record to grow into one.
...
extrinsic relative to a rig that is not in the file is not a pose at all, and a reader that fell back
to treating it as a scene-frame pose would place every sensor at the rig's origin — plausible,
...
`fy` MUST be non-zero and `width_px` and `height_px` MUST be non-zero. When `camera_model` **is** `0`
— a lidar, a radar, an IMU, or a camera whose intrinsics were not recovered — `width_px`,
...
`rotation` and `translation` mean what they mean in §5.15.3: `p_scene = R(rotation) * p_rig +
translation`, with the quaternion in `xyzw` order.
...
#### 5.15.5 Still reserved — opcodes `0x23`–`0x2F`
...
| §5.15 added: Coordinate Frame `0x20`, Sensor Calibration `0x21` and Rig Trajectory `0x22`, all optional | rule added              |
...
opcodes they define were reserved and unemitted, so no file in the world carries one; every
conformance variant that existed before them still encodes to the same bytes and decodes to the same
meaning; and because all three records are optional with no Header flag announcing them (§5.15.1), a
file that does not want provenance is byte-identical to the file it would have been. What changes is
what a **new** file may say. The precedence row is the one with teeth: a producer that has been
writing `coordinate_system` as metadata keeps working unchanged, and only a file carrying both the
key and the record has a question to answer — which is why the answer is stated before anyone can
write that file rather than after.
