...
keyframe-delta and 8 object-layer variants in their own subdirectories, over two read paths
(streamed and indexed). A language takes the variants it declares support for, and what it declines
is what this table records. Python, the fully supporting reference layer, passes 135 checks; the
five stacked language layers each pass 32 while explicitly declining the 52 aggregate-bearing
