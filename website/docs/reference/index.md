...
keyframe-delta and 7 object-layer variants in their own subdirectories, over two read paths
(streamed and indexed). A language takes the variants it declares support for, and what it declines
is what this table records. Python, the fully supporting reference layer, passes 133 checks; the
five stacked language layers each pass 32 while explicitly declining the 51 aggregate-bearing
