...
#: The three optional exact-label lanes. Complete states and births default an omitted
#: lane to zero; updates carry it forward when absent and replace it absolutely when
#: present. They are labels, not coordinates on a quantization grid, so subtraction has
#: no semantic meaning (spec §6.1 and §11.3).
...
ABSOLUTE_IN_UPDATE = frozenset({op.A_ROTATION_INDEX, op.A_ROTATION}) | ZERO_DEFAULT_IDENTITY
    ...
    *,
    ...
    if update_ids.size:
        ...
        for attribute, raw in update_bins.items():
            ...
            if attribute not in state.bins and attribute not in ZERO_DEFAULT_IDENTITY:
                raise _refuse(
            ...
            if attribute not in state.bins:
                # A physically absent identity lane is a logical population-length zero
                # column. Materialize it only when an update supplies a non-zero row.
                state.bins[attribute] = np.zeros((state.count, *delta.shape[1:]), dtype=np.int64)
