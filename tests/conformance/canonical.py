...
two correct decoders to disagree. Per-gaussian rows, aggregates and the spherical harmonic
digest use the rounded content order defined by `_stable_order`.
...
aggregates add in that same emitted order rather than resident order.
    ...
    *,
    ...
    return {
        ...
        **(
        ...
        "aggregate": {
            "positionSum": [num(value) for value in total_pos],
