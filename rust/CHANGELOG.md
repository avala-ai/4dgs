...
- **Streamed readers and validators refuse defined front matter after state begins.** The
  ...
  the placement error. `Error::late_front_matter_records` exposes those two sites without parsing
  the diagnostic. Unknown and private records keep their existing position-independent handling,
  while indexed open retains its bounded early-stop exemption.
...
- **Readers refuse a finite `Quantization.step_time` at or below zero by name.** Both front-to-back
  ...
  evaluate `0 / 0`; a negative pitch reverses the grid and gives different readers room to
  reconstruct different state.
